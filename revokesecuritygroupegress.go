...
func (p *RevokeSecurityGroupEgressParameter) SetId(s string) {
	p.Id.String = s
	p.Id.Valid = true
}
func (p *RevokeSecurityGroupEgressParameter) ToMap() map[string]string {
	m := map[string]string{}
	if p.Id.Valid {
...
func (c *Client) RevokeSecurityGroupEgress(p RevokeSecurityGroupEgressParameter) (Result, error) {
	var ret Result
	b, err := c.Request("revokeSecurityGroupEgress", p.ToMap())
	if err != nil {
